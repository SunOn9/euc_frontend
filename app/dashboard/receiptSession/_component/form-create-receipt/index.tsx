...
import { receiptCreate } from "@/service/api/receipt/create";
...
export default function ReceiptForm(props: Props) {
  ...
  const handleReceiptCreate = (values: CreateReceiptRequest) => {
    setIsLoading(true);
    values.receiptSessionId = props.id;
    receiptCreate(values)
      .then((res) => {
        ...
        queryClient.invalidateQueries(["receiptSessionDetail"]);
        props.onClose();
        props.onChange();
      })
      .catch((err) => {
  ...
  return (
    <Formik
      initialValues={CreateReceiptRequest.create({
        method: EnumProto_MoneyMethod.UNRECOGNIZED,
      })}
      validationSchema={ValidateSchema}
      onSubmit={(values) => {
        handleReceiptCreate(values);
      }}
    >
      {({
        values,
