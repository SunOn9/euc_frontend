...
type Props = {
  showFilter: boolean;
  setReceiptSessionSearchParam: React.Dispatch<
    React.SetStateAction<GetReceiptSessionConditionRequest>
  >;
};

export default function ReceiptSessionFilterForm(props: Props) {
  ...
  const onFinish = async (values: SearchDataType) => {
    const request = GetReceiptSessionConditionRequest.create({
    ...
    props.setReceiptSessionSearchParam(request);
  };

  return (
    <Formik
      initialValues={GetReceiptSessionConditionRequest.create({
